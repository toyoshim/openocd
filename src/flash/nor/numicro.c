...
#define NUC_M0_FLASH_MSB4 0x20171011
#define NUC_M23_FLASH_VERSION 0x50003FFC
#define NUC_M23_FLASH_MSB5 0x20201130
...
static const struct numicro_cpu_type NuMicroParts[] = {
	...
	{"M463SJHAE", 0x01B46310, NUMICRO_BANKS_OFFSET(1024*1024, 0*1024, 8*1024, 16)},
	{"M463H3SJHAN", 0x01B46B10, NUMICRO_BANKS_OFFSET(1024*1024, 0*1024, 8*1024, 16)},
	{"M463SIHAE", 0x01B46311, NUMICRO_BANKS_OFFSET(1024*1024, 0*1024, 8*1024, 16)},
	{"M467KJHAE", 0x01B46740, NUMICRO_BANKS_OFFSET(1024*1024, 0*1024, 8*1024, 16)},
	{"M463KJHAE", 0x01B46340, NUMICRO_BANKS_OFFSET(1024*1024, 0*1024, 8*1024, 16)},
	{"M463H3KJHAN", 0x01B46B40, NUMICRO_BANKS_OFFSET(1024*1024, 0*1024, 8*1024, 16)},
	{"M463KIHAE", 0x01B46341, NUMICRO_BANKS_OFFSET(1024*1024, 0*1024, 8*1024, 16)},
	...
	{"M464SGCAE", 0x01C46410, NUMICRO_BANKS_OFFSET(1024*1024, 0*1024, 8*1024, 16)},
	{"M463AGCAE", 0x01C46380, NUMICRO_BANKS_OFFSET(1024*1024, 0*1024, 8*1024, 16)},
	{"M464AGCAE", 0x01C46480, NUMICRO_BANKS_OFFSET(1024*1024, 0*1024, 8*1024, 16)},
...
static int numicro_get_arm_arch(struct target *target)
{
	...
	if (armv7m->arm.arch == ARM_ARCH_V6M) {
		...
		uint32_t reg_value;
		int retval = target_read_u32(target, NUC_M0_FLASH_VERSION, &reg_value);
		if (retval == ERROR_OK && reg_value == NUC_M0_FLASH_MSB4) {
		...
		uint32_t reg_value;
		int retval = target_read_u32(target, NUC_M23_FLASH_VERSION, &reg_value);
		if (retval == ERROR_OK && reg_value == NUC_M23_FLASH_MSB5) {
			m_addressMinusOffset = 0;
		}
		else {
		m_addressMinusOffset = 0x10000000;
		...
		if (armv7m->arm.core_type == ARM_CORE_TYPE_SEC_EXT) {
			// FIXME: M2351/M2354
			if (false/*armv7m->arm.is_armv8mSecureInvasiveDebugAllowed*/) {
		...
		uint32_t offset, uint32_t count)
{
	...
	if (armv7m->arm.arch == ARM_ARCH_V6M) {
		...
		uint32_t reg_value;
		retval = target_read_u32(target, NUC_M0_FLASH_VERSION, &reg_value);
		if (retval == ERROR_OK && reg_value == NUC_M0_FLASH_MSB4) {
	...
	if (strcmp(m_target_name, "M480") == 0 && bSPIMFlashWrite) {
		...
		if (strcmp(m_target_name, "M471") == 0 && bDFMCFlashWrite) {	// M471 data flash DFMC erase
			...
			for (i = first; i <= last; i++) {
				...
				LOG_DEBUG("address: 0x%x, bank->base: " TARGET_ADDR_FMT ", bank->sectors[i].offset: 0x%x,  %d", address, bank->base, bank->sectors[i].offset, i);
				address = (bank->base + bank->sectors[i].offset);
				buf_set_u32(reg_params[0].value, 0, 32, address);
		...
		uint32_t offset, uint32_t count)
{
	...
	if (m_M23SecureDebugState == NUMICRO_M23_SECURE_DEBUG_S) {
		...
		LOG_DEBUG("base: " TARGET_ADDR_FMT ",offset: 0x%" PRIx32 "", bank->base, offset);
		if (bank->base + offset < NUMICRO_LDROM_BASE) {
			// APROM
	...
	for (size_t i = 0; i < cpu->n_banks; i++) {
		...
		if (bank->base == cpu->bank[i].base) {
			*flash_size = cpu->bank[i].size;
			LOG_INFO("bank base = " TARGET_ADDR_FMT ", size = 0x%08" PRIx32 "", bank->base, *flash_size);
			return ERROR_OK;
		}
	}
	return ERROR_FLASH_OPERATION_FAILED;
...
static int numicro_probe(struct flash_bank *bank)
{
	...
	if (armv7m->arm.arch == ARM_ARCH_V6M) { /* M0 */
	...
	else if (armv7m->arm.arch == ARM_ARCH_V8M) { /* M23 */
		if ((cpu->partid & 0xFFFFFF00) == 0x01812600/* NUC1262 */ ||
			 armv7m->arm.core_type == ARM_CORE_TYPE_SEC_EXT) {
...
COMMAND_HANDLER(numicro_handle_M2351_erase_command)
{
	...
	retval = ERROR_FAIL;  // FIXME: nulink_usb_M2351_erase();
	if (retval != ERROR_OK) {
